 ...
 *
 * Given the current design as structured JSON (walls + devices + scale),
 * Claude returns a punch list of findings: blind spots, FOV redundancies,
 * missing coverage at entries, sensor gaps, compliance considerations.
 *
 * Each finding includes a severity, the affected location (when point-
 * specific), a clear one-sentence problem statement, and a suggested fix
 * the client UI renders as a one-click "apply" or "dismiss" action.
 *
 * Why structured-JSON-not-image: we already have exact wall and device
 * geometry, and Claude can reason about distances/coverage from the data
 * directly. An image would cost more tokens for no additional clarity.
 */
...
interface AdvisorRequestBody {
  ...
  /** Optional building context — e.g. "medical clinic" / "warehouse" /
      "K-12 school" — used to apply industry-specific compliance rules. */
  buildingType?: string;
}
...
export interface AdvisorFinding {
  ...
  title: string;
  description: string;
  /** Optional location in floor-plan pixels — used to draw a marker on the canvas. */
  location?: { x: number; y: number };
  /** What the user should do — wired up to a one-click apply in the UI. */
...
export async function POST(request: NextRequest) {
  ...
  // Compact textual summary of the floor — easier for Claude to reason about
  // than raw JSON without losing fidelity.
      ...
      for (const tool of toolUses) {
        ...
        if (tool.name === "report_finding") {
          ...
          findings.push({
            ...
            title:
              typeof input.title === "string" ? input.title : "Finding",
            description:
  ...
  const result: AdvisorResponse = { summary, findings, usage };
...
function formatFloorForAdvisor(body: AdvisorRequestBody): string {
  ...
  lines.push(`Walls (${floor.walls.length}) — each is a line segment from (startX, startY) to (endX, endY) in pixels:`);
  ...
  for (const d of floor.devices) {
    ...
    const fov =
      d.fovDegrees != null
        ...
        : "";
    const range =
      d.rangeMeters != null ? ` · ${d.rangeMeters} m range` : "";
